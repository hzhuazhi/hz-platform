...
//            }
    ...
    /**
     * @Description: 思维付支付-代收
     * @param response
     * @return com.gd.chain.common.utils.JsonResult<java.lang.Object>
     * @author yoko
     * @date 2019/11/25 22:58
     * local:http://localhost:8092/platform/data/swfIn
     * http://localhost:8092/platform/data/swfIn
     *
     * {"state":"1","msg":"msg1","ordernum":"ordernum1","outordernum":"outordernum1","amount":5000, "respTxnTime":"respTxnTime1","paytype":"paytype1","attach":"attach1","listid":"listid1","sign":"sign1"}
     */
    @RequestMapping(value = "/swfIn", method = {RequestMethod.POST})
    public String swfIn(HttpServletRequest request, HttpServletResponse response, RequestSwf requestModel) throws Exception{
        String ip = StringUtil.getIpAddress(request);
        String data = "";
//        RequestFine requestModel = new RequestFine();
        try{
...
//            requestModel = (RequestFine) getRootBean(RequestFine.class, request);
            ...
            if (requestModel.ordernum == null){
                return "0";
            }
            if (requestModel.outordernum == null){
                return "0";
            }
            if (requestModel.amount == null){
                return "0";
            ...
            if (StringUtils.isBlank(requestModel.state)){
                ...
                if (!requestModel.state.equals("1")){
                    return "1";// 只收取成功数据
                }
            }


            log.info("---------------swf:ordernum:" + requestModel.ordernum + "payOrderId:" + requestModel.outordernum);
            String resStr = JSON.toJSONString(requestModel);
            log.info("swf-----------all-----data:" + resStr);
            String amount = StringUtil.getBigDecimalDivide(String.valueOf(requestModel.amount), "100.00");
            String sy_pay_amount = StringUtil.getBigDecimalDivide(String.valueOf(requestModel.amount), "100.00");
            String total_amount = amount;
            String pay_amount = sy_pay_amount;
            // 查询此数据属于哪个订单
            ChannelDataModel channelDataModel = new ChannelDataModel();
            channelDataModel.setMyTradeNo(requestModel.ordernum);
            channelDataModel = (ChannelDataModel) ComponentUtil.channelDataService.findByObject(channelDataModel);
            if (channelDataModel == null){
                return "0";
            }
...
//            }
            // 更新状态
            ReceivingAccountDataModel receivingAccountDataModel = new ReceivingAccountDataModel();
            receivingAccountDataModel.setMyTradeNo(requestModel.ordernum);
            receivingAccountDataModel.setIsOk(2);
            ComponentUtil.receivingAccountDataService.update(receivingAccountDataModel);
            //组装上游数据
            DataCoreModel dataCoreModel = HodgepodgeMethod.assembleDataCoreSwf(requestModel, channelDataModel, channelGewayModel, total_amount, serviceCharge, actualMoney, tradeStatus,
                    pay_amount, payActualMoney, moneyFitType, channelDataModel.getChannelGewayId(), channelDataModel.getProfitType());
            int num = ComponentUtil.dataCoreService.add(dataCoreModel);
            if (num > 0){
                return "1";
            ...
            Map<String,String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            // #添加异常
            log.error(String.format("this DataCoreController.swfIn() is error , the all data=%s!", data));
            e.printStackTrace();
            return JSON.toJSONString(map);
        }
    }
    ...
    /**
     * @Description: 花呗通支付-代收
     * @param response
     * @return com.gd.chain.common.utils.JsonResult<java.lang.Object>
     * @author yoko
     * @date 2019/11/25 22:58
     * local:http://localhost:8092/platform/data/hbtIn
     * http://localhost:8092/platform/data/hbtIn
     *
     * {"p1_merchantno":"p1_merchantno1","p2_amount":"50.00","p3_orderno":"p3_orderno1","p4_status":"2","p5_producttype":"p5_producttype1", "p6_requesttime":"p6_requesttime1","p7_goodsname":"p7_goodsname1","p8_tradetime":"p8_tradetime1","p9_porderno":"p9_porderno1","sign":"sign1"}
     */
    @RequestMapping(value = "/hbtIn", method = {RequestMethod.GET})
    public String hbtIn(HttpServletRequest request, HttpServletResponse response, RequestHbt requestModel) throws Exception{
        String ip = StringUtil.getIpAddress(request);
        String data = "";
//        RequestFine requestModel = new RequestFine();
        try{
...
//            requestModel = (RequestFine) getRootBean(RequestFine.class, request);
            ...
            if (requestModel.p3_orderno == null){
                return "no";
            }
            if (requestModel.p9_porderno == null){
                return "no";
            }
            if (requestModel.p2_amount == null){
                return "no";
            }
            if (StringUtils.isBlank(requestModel.p4_status)){
                return "SUCCESS";// 只收取成功数据
            }else {
                if (!requestModel.p4_status.equals("2")){
                    return "SUCCESS";// 只收取成功数据
                }
            }


            log.info("---------------hbt:p3_orderno:" + requestModel.p3_orderno + "p9_porderno:" + requestModel.p9_porderno);
            String resStr = JSON.toJSONString(requestModel);
            log.info("hbt-----------notify-----data:" + resStr);
            String amount = requestModel.p2_amount;
            String sy_pay_amount = requestModel.p2_amount;
            String total_amount = amount;
            String pay_amount = sy_pay_amount;
            // 查询此数据属于哪个订单
            ChannelDataModel channelDataModel = new ChannelDataModel();
            channelDataModel.setMyTradeNo(requestModel.p3_orderno);
            channelDataModel = (ChannelDataModel) ComponentUtil.channelDataService.findByObject(channelDataModel);
            if (channelDataModel == null){
                return "no";
            }
...
//            }
            // 更新状态
            ReceivingAccountDataModel receivingAccountDataModel = new ReceivingAccountDataModel();
            receivingAccountDataModel.setMyTradeNo(requestModel.p3_orderno);
            receivingAccountDataModel.setIsOk(2);
            ComponentUtil.receivingAccountDataService.update(receivingAccountDataModel);
            //组装上游数据
            DataCoreModel dataCoreModel = HodgepodgeMethod.assembleDataCoreHbt(requestModel, channelDataModel, channelGewayModel, total_amount, serviceCharge, actualMoney, tradeStatus,
                    pay_amount, payActualMoney, moneyFitType, channelDataModel.getChannelGewayId(), channelDataModel.getProfitType());
            int num = ComponentUtil.dataCoreService.add(dataCoreModel);
            if (num > 0){
                return "SUCCESS";
            ...
            Map<String,String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            // #添加异常
            log.error(String.format("this DataCoreController.hbtIn() is error , the all data=%s!", data));
            e.printStackTrace();
            return JSON.toJSONString(map);
        }
    }
     ...
     *
     * {"merId":"merId1","orderId":"orderId1","sysOrderId":"sysOrderId1","desc":"desc1","orderAmt":"50.00","status":1, "nonceStr":"nonceStr1","sign":"sign1","attch":"attch1"}
     */
    @RequestMapping(value = "/ybIn", method = {RequestMethod.POST})
    public String ybIn(HttpServletRequest request, HttpServletResponse response, RequestYb requestModel) throws Exception{
        String ip = StringUtil.getIpAddress(request);
        String data = "";
//        RequestFine requestModel = new RequestFine();
        try{
            ...
            if (requestModel.sysOrderId == null){
                return "no";
            }
            if (requestModel.orderAmt == null){
                return "no";
            }
            ...
            log.info("---------------yb:orderId:" + requestModel.orderId + ",sysOrderId:" + requestModel.sysOrderId);
            String resStr = JSON.toJSONString(requestModel);
            log.info("yb-----------all-----data:" + resStr);
            String total_amount = requestModel.orderAmt;
            String pay_amount = requestModel.orderAmt;
            // 查询此数据属于哪个订单
            ChannelDataModel channelDataModel = new ChannelDataModel();
            channelDataModel.setMyTradeNo(requestModel.orderId);
            channelDataModel = (ChannelDataModel) ComponentUtil.channelDataService.findByObject(channelDataModel);
...
//            }
            ...
            ComponentUtil.receivingAccountDataService.update(receivingAccountDataModel);
            //组装上游数据
            DataCoreModel dataCoreModel = HodgepodgeMethod.assembleDataCoreYb(requestModel, channelDataModel, channelGewayModel, total_amount, serviceCharge, actualMoney, tradeStatus,
                    pay_amount, payActualMoney, moneyFitType, channelDataModel.getChannelGewayId(), channelDataModel.getProfitType());
            int num = ComponentUtil.dataCoreService.add(dataCoreModel);
            if (num > 0){
                return "success";
            ...
            Map<String,String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            // #添加异常
            log.error(String.format("this DataCoreController.ybIn() is error , the all data=%s!", data));
            e.printStackTrace();
            return JSON.toJSONString(map);
        }
    }
     ...
     * @author yoko
     * @date 2019/11/25 22:58cakeOut
     * local:http://localhost:8092/platform/data/mgIn
     * http://localhost:8092/platform/data/mgIn
     *
     * {"memberid":"memberid1","orderid":"orderid1","amount":"50.00","transaction_id":"transaction_id1","datetime":"datetime1","returncode":"00","attach":"attach1","sign":"sign1"}
     */
    @RequestMapping(value = "/mgIn", method = {RequestMethod.POST})
    public String mgIn(HttpServletRequest request, HttpServletResponse response, RequestMg requestModel) throws Exception{
        String ip = StringUtil.getIpAddress(request);
        String data = "";
//        RequestFine requestModel = new RequestFine();
        try{
            ...
            log.info("---------------mg:orderid:" + requestModel.orderid + ",transaction_id:" + requestModel.transaction_id);
            String resStr = JSON.toJSONString(requestModel);
            log.info("mg-----------all-----data:" + resStr);
            String total_amount = requestModel.amount;
            String pay_amount = requestModel.amount;
            // 查询此数据属于哪个订单
            ChannelDataModel channelDataModel = new ChannelDataModel();
...
//            }
            ...
            ComponentUtil.receivingAccountDataService.update(receivingAccountDataModel);
            //组装上游数据
            DataCoreModel dataCoreModel = HodgepodgeMethod.assembleDataCoreMg(requestModel, channelDataModel, channelGewayModel, total_amount, serviceCharge, actualMoney, tradeStatus,
                    pay_amount, payActualMoney, moneyFitType, channelDataModel.getChannelGewayId(), channelDataModel.getProfitType());
            int num = ComponentUtil.dataCoreService.add(dataCoreModel);
            if (num > 0){
                return "OK";
            ...
            Map<String,String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            // #添加异常
            log.error(String.format("this DataCoreController.mgIn() is error , the all data=%s!", data));
            e.printStackTrace();
            return JSON.toJSONString(map);
        }
    }
     ...
     * @author yoko
     * @date 2019/11/25 22:58cakeOut
     * local:http://localhost:8092/platform/data/dfzfIn
     * http://localhost:8092/platform/data/dfzfIn
     *
     * {"amount":"50.00","bankCode":"bankCode1","outOrderNo":"outOrderNo1","extend":"extend1","responseMsg":"responseMsg1","goodsName":"goodsName1","mchId":"mchId1","responseState":"1","orderNo":"orderNo1","payTime":"payTime1","signType":"signType1","sign","sign1"}
     */
    @RequestMapping(value = "/dfzfIn", method = {RequestMethod.POST})
    public String dfzfIn(HttpServletRequest request, HttpServletResponse response, RequestDfzf requestModel) throws Exception{
        String ip = StringUtil.getIpAddress(request);
        String data = "";
//        RequestFine requestModel = new RequestFine();
        try{
            ...
            if (requestModel.outOrderNo == null){
                return "no";
            }
            if (requestModel.amount == null){
                return "no";
            }
            if (!requestModel.responseState.equals("1")){
                return "SUCCESS";// 只收取成功数据
            }


            log.info("---------------dfzf:orderNo:" + requestModel.orderNo + ",outOrderNo:" + requestModel.outOrderNo);
            String resStr = JSON.toJSONString(requestModel);
            log.info("dfzf-----------all-----data:" + resStr);
            String total_amount = requestModel.amount;
            String pay_amount = requestModel.amount;
            // 查询此数据属于哪个订单
            ChannelDataModel channelDataModel = new ChannelDataModel();
...
//            }
            ...
            ComponentUtil.receivingAccountDataService.update(receivingAccountDataModel);
            //组装上游数据
            DataCoreModel dataCoreModel = HodgepodgeMethod.assembleDataCoreDfzf(requestModel, channelDataModel, channelGewayModel, total_amount, serviceCharge, actualMoney, tradeStatus,
                    pay_amount, payActualMoney, moneyFitType, channelDataModel.getChannelGewayId(), channelDataModel.getProfitType());
            int num = ComponentUtil.dataCoreService.add(dataCoreModel);
            if (num > 0){
                return "SUCCESS";
            ...
            Map<String,String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            // #添加异常
            log.error(String.format("this DataCoreController.dfzfIn() is error , the all data=%s!", data));
            e.printStackTrace();
            return JSON.toJSONString(map);
        }
    }
...
//            }
            ...
            ComponentUtil.receivingAccountDataService.update(receivingAccountDataModel);
            //组装上游数据
            DataCoreModel dataCoreModel = HodgepodgeMethod.assembleDataCoreZz(requestModel, channelDataModel, channelGewayModel, total_amount, serviceCharge, actualMoney, tradeStatus,
                    pay_amount, payActualMoney, moneyFitType, channelDataModel.getChannelGewayId(), channelDataModel.getProfitType());
            int num = ComponentUtil.dataCoreService.add(dataCoreModel);
            if (num > 0){
                return "success";
            ...
            Map<String,String> map = ExceptionMethod.getException(e, ServerConstant.PUBLIC_CONSTANT.SIZE_VALUE_TWO);
            // #添加异常
            log.error(String.format("this DataCoreController.zzIn() is error , the all data=%s!", data));
            e.printStackTrace();
            return JSON.toJSONString(map);
        }
    }
